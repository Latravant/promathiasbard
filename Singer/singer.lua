...
default = {
    ...
    min_ws=20,
    max_ws=99,
    box={bg={visible=true},text={size=10},pos={x=1650,y=0}},
}
...
windower.register_event('addon command', function(...)
   ...
   elseif commands[1]:startswith('dummy') then
    ...
    elseif commands[1] == 'help' then
		print(' sing haste <name> [on|off]        -  Add or remove names of players for Haste cycle.')
		print(' sing refresh <name> [on|off]      -  Add or remove names of players for Refresh cycle.')
		print(' sing pianissimo [on|off]          -  Toggle pianissimo usage, can be shortened to p.')
		print(' sing nightingale [on|off]         -  Toggle nightingale usage, can be shortened to n.')
		print(' sing troubadour [on|off]          -  Toggle troubadour usage, can be shortened to t.')
		print(' sing <buff> [n]                   -  Set aoe buff song to x[n] or off.*')
		print(' sing <buff> [n] [name]            -  Set pianissimo song type to x[n] or off for [name].*')
		print(' sing marcato <song>               -  Set song to use following marcato.**')
		print(' sing aoe [slot|name] [on|off]     -  Set party slots to monitor for aoe range.')
		print(' sing save [list] [name]           -  Save settings, if <list> is provided will save current songs to playlist.')
		print(' sing <order> <song> [name]        -  Set songs to be used in specified order.**')
		print(' sing <order> <clear> [name]       -  Remove song from specified slot.')
		print(' sing playlist save <list> [name]  -  Saves current songs to playlist.')
		print(' sing playlist <list|clear> [name] -  Loads songs from a previously set playlist. (clear is an empty playlist to remove all songs)')
		print(' sing clear <name|aoe>				-  Clear song list for name')
	end
    bard_status:text(display_box())
end)
